...
namespace WebApp.DataAccess
{
    public class ResidentialDal
    {
        ...
        {
            string query = $"Insert into Residentals (RealEstateId,ResidentalType,SellType,Square,Address,Age,FloorNumber,Heating,Balcony,Furnished) VALUES  ('{residential.RealEstateId}'," +
        ...
        {
            string query = "select * from Residentals";

            return DbTools.Connection.ReadResidentials(query);
        }
        public Residential GetResidentialById(int id)
        {
            string query = $"select * from Residentals where ID ={id};";
            return DbTools.Connection.ReadResidentials(query)[0];
        }
        public bool Update(Residential residential)
        {
            string query = $"Update Residentals set RealEstateID='{residential.RealEstateId}',SellType='{residential.SellType}', Square='{residential.Square}',Age='{residential.RealEstateId}',FloorNumber='{residential.FloorNumber}',Heating='{residential.Heating}',Balcony='{residential.Balcony}',Furnished='{residential.Furnished}',ResidentialType='{residential.ResidentialType}',AddressID='{residential.AddressId}', where AddressID='{residential.AddressId}';";
            return DbTools.Connection.Execute(query);
        }
        public bool Delete(Residential residential)
        {
            string query = $"Delete from Residentals where Id ={residential.RealEstateId};";
            return DbTools.Connection.Execute(query);
        }
